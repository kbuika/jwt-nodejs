...
// generate user copy without their password
...
// recover user based on username
...
export const getAllUsers = (): IUser[] => {
    return Object.values(users).map((el) => generateSafeCopy(el))
}
...
export const createUser = async (username: string, password: string, role: Roles): Promise<IUser> => {
    ...
    if(getUserByUsername(username) != undefined) throw new ClientError("Username is already taken")
...
export const updateUser = (id: string, username: string, role: Roles): IUser => {
    ...
    if(userIdWithUsername !== undefined && userIdWithUsername !== id) throw new ClientError(`username is already taken`)
